        ...
        if h_rv : RV t₁₂ then
          some ⟨⟨θ₁.val.wIdx + θ₂.val.wIdx, t₁₂⟩, h_rv,
            by 
              ...
              case isTrue h_eq => injection h_c with h_val; rw [← h_val]; exact θ₁.2.2.1,
            by
              unfold concat at h_c
              split at h_c; case isFalse => contradiction
              case isTrue h_eq => injection h_c with h_val; rw [← h_val]; exact θ₂.2.2.2
          ⟩ else none
  cost θ := 
...
def projQuotient (X : Type) [DecidableEq X]
    ...
    (hiddenSystemQuotient X witness RV).G → (observableSystemQuotient X witness RV h_id).V :=
  fun θ => Quotient.mk' θ
...
def systemFromV1Quotient (X : Type) [DecidableEq X]
  ...
  proj := projQuotient X witness RV h_id
    ...
    Assumptions (systemFromV1Quotient X witness RV h_id) :=
{
  obs_assoc := by
    ...
    simp [observableSystemQuotient, systemFromV1Quotient] at *
    rw [← h12, ← h23, ← h123a]
    congr
  obs_id_right := by
    intro R
  ...
  id_fiber_zero := by
    intro ξ hξ
    simp [projQuotient, mem_Fiber, systemFromV1Quotient] at hξ
    simp [hiddenSystemQuotient, hξ]
  hidden_cost_add := by
    intro ξ₂ ξ₁ ξ hcomp
    simp [hiddenSystemQuotient] at *
    match hr : concat ξ₁.val.trace ξ₂.val.trace with
    | some t₁₂ =>
      rw [hr] at hcomp
      ...
      case isTrue h_rv =>
        ...
        · -- Case: composite is emptyTrace. Then both components must be emptyTrace.
          ...
          have h1 : ξ₁.val.trace = emptyTrace witness := Trace.ext _ _ rfl rfl h_steps.1
          have h2 : ξ₂.val.trace = emptyTrace witness := Trace.ext _ _ rfl rfl h_steps.2
          simp [h1, h2]
        · -- Case: composite is not emptyTrace.
          ...
          · -- Both empty. Then composite is empty, contradiction.
            have h1_eq : ξ₁.val.trace = emptyTrace witness := Trace.ext _ _ rfl rfl h1
            have h2_eq : ξ₂.val.trace = emptyTrace witness := Trace.ext _ _ rfl rfl h2
  ...
  fiber_decomp := by
    ...
    match hr : concat θ1.val.trace θ2.val.trace with
    | some t12 =>
        rw [hr] at hc
        ...
        case isTrue h_rv =>
          ...
          use ⟨⟨0, θ2.val.trace⟩, θ2.2⟩, ⟨⟨ξ.val.wIdx, θ1.val.trace⟩, θ1.2⟩
          constructor
          ...
          · constructor
            · rw [h_eq]
              simp [Rel]
            · simp [Rel]
    | none => rw [hr] at hc; contradiction
}
