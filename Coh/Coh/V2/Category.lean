...
def HomLE (S : System) (A : Assumptions S) (X : Type v) (V : X → ℝ)
    {x y : X}
    (f g : CertifiedMor S A V x y) : Prop :=
...
instance instHomPartialOrder (S : System) (A : Assumptions S) (X : Type v) (V : X → ℝ) {x y : X} :
    PartialOrder (CertifiedMor S A V x y) where
...
theorem assoc_certified
    ...
    {w x y z : X}
    (f : CertifiedMor S A V w x)
    (g : CertifiedMor S A V x y)
    (h : CertifiedMor S A V y z)
    {R₂₁ R₃₂ R₃₂₁a R₃₂₁b : S.Obs.V}
    ...
    compose V (compose V f g h₁₂) h h₁₂₃a =
    compose V f (compose V g h h₂₃) h₁₂₃b := by
  apply CertifiedMor.ext
...
theorem id_right_certified
    ...
    compose V (idMor V x) f hcomp = f := by
  apply CertifiedMor.ext
...
theorem id_left_certified
    ...
    compose V f (idMor V y) hcomp = f := by
  apply CertifiedMor.ext
...
def certifiedCategory
  ...
  have id_right_law : ∀ {x y : X} (f : CertifiedMor S A V x y),
      compObj (idObj x) f = f := by
    intro x y f
    let p := chooseComp (idObj x) f
    apply id_right_certified S A X V f p.2
  have id_left_law : ∀ {x y : X} (f : CertifiedMor S A V x y),
  ...
  have comp_monotone_law : ∀ {x y z : X} {f₁ f₂ : CertifiedMor S A V x y} {g₁ g₂ : CertifiedMor S A V y z},
